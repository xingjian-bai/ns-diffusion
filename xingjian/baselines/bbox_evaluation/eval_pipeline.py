...
class EvalAdaptedDataset(Dataset):
    ...
    def __getitem__(self, index):
        clean_image, objects, relations, labels, generated_prompt, raw_image, annotated_image_tensor, relations_ids, _, _, _ = self.data[index]
...
def evaluate(dataset_type, metric, threshold = 0.5, wandb_drawer = None, args = None):
    ...
    if dataset_type == 2 and args.pure_synthetic == False:
        dataset = RelationalDataset2O(num_upperbound=args.num_upperbound)
    elif dataset_type == 3 and args.pure_synthetic == False:
        dataset = RelationalDataset3O(num_upperbound=args.num_upperbound)
    elif dataset_type == 4 and args.pure_synthetic == False:
        dataset = RelationalDataset4O(num_upperbound=args.num_upperbound)
    elif dataset_type == 5 and args.pure_synthetic == False:
        dataset = RelationalDataset5O(num_upperbound=args.num_upperbound)
    elif dataset_type == 8 and args.pure_synthetic == False:
        dataset = RelationalDataset8O(num_upperbound=args.num_upperbound)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trainer_name', type=str, default='bbox2.0', help='name of the trainer')
