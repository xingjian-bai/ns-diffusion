...
class Unet_conditional(Unet):
    ...
    def forward(self, cond, x, time, x_self_cond=None):
        ...
        print(f"shape of time: {time.shape}, cond: {cond.shape}")
...
class BiCondUnet(nn.Module):
    def __init__(self, 
                 ...
                 no_rel = False, no_obj = False, mask_bbox = False,):
        super().__init__()
        self.no_rel = no_rel
        self.no_obj = no_obj
    ...
    def name(self):
        return f"Bi-CondUnet{self.composition_type}" + \
                ("" if not self.no_rel else "[no_rel]") + \
                ("" if not self.no_obj else "[no_obj]") + \
                ("" if not self.mask_bbox else "[mask_bbox]")
    
    def forward(self, conds, x, time, x_self_cond=None):
        """
        output_batch: (batch, obj_num, 4), the calculated noises for each object
        """
        (objects, relations, relations_ids) = conds
        batch_size = len(objects)
        obj_num = objects.shape[1]
        ...
        if not self.no_obj:
            for i in range(obj_num):
                obj_out = self.obj_denoise(objects[:, i], x, time)

                # Create a binary mask for each image in the batch
                if self.mask_bbox:
                    mask = torch.zeros(batch_size, 1, 128, 128).to(x.device)
                    for j in range(batch_size):  # For each image in the batch
                        bbox = BoundingBox(objects[j, i, -4:])
                        xmin, ymin, xmax, ymax = bbox.normalized_output(normalized_range=(0, 128), force_int=True)
                        mask[j, :, ymin:ymax, xmin:xmax] = 1
                        # print(f"in mask_bbox, ", xmin, ymin, xmax, ymax)
                    obj_out *= mask.expand_as(obj_out)

                output_batch += obj_out
