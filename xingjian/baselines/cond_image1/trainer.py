...
import sys
sys.path.append('../')
from dataset_clevr_ryan import BoundingBox, gen_rand_bbox, gen_rand_object
from utils import *
            ...
            while self.step < step_limit:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    if self.dataset_type == 'CLEVR_1O' or self.dataset_type == 'CLEVR_2O' or self.dataset_type == 'CLEVR_3O' or self.dataset_type == 'CLEVR_4O':
                        ...
                        objects_combined = torch.cat((objects, bboxes), dim=-1)
                    ...
                    with self.accelerator.autocast():
                        conds = (objects_combined, relations, relations_ids)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and divisible_by(self.step, self.save_and_sample_every):
                        ...
                        def visualize(conds, text = "generated", images = None):
                            print(f"visualizing {text} images")
                            combined_objects, relations, relations_ids = conds
                            ...
                            conds = (combined_objects, relations, relations_ids)
                            ...
                            for (i, image) in enumerate(all_sampled_images):
                                ...
                                if images is not None:
                                    ...
                                    original_image = Image.new("RGB", (self.image_size, self.image_size), color = "white")
                        ...
                        visualize(conds, "generated", images = images)

                        eval_data = next(self.eval_dl)
                        eval_images, eval_objects, eval_bboxes, eval_relations, eval_relations_ids = eval_data
                        eval_images, eval_objects, eval_bboxes = eval_images.to(device), eval_objects.to(device), eval_bboxes.to(device)
                        ...
                        eval_objects_combined = torch.cat((eval_objects, eval_bboxes), dim=-1)
                        visualize((eval_objects_combined, eval_relations, eval_relations_ids), "eval")

                        


                        random_combined_objects = torch.zeros((self.eval_batch_size, 1, 8))
                        for i in range(self.eval_batch_size):
                            ...
                            image_objects = torch.cat((random_object, random_bbox), dim=-1).unsqueeze(0)
                            random_combined_objects[i] = image_objects
                        random_combined_objects = torch.tensor(random_combined_objects)

                        random_relations = [[] for _ in range(self.eval_batch_size)]
                        random_relations_ids = [[] for _ in range(self.eval_batch_size)]
                        # convert to device
